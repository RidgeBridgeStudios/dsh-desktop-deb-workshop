...
| `usr/share/dsh-desktop/lib/plugin-manager/recovery.mjs` | Crash diagnosis, batch recovery planning & repairs | `createRecoveryService()`, `resolveRecoveryPlan()`, `applyRecoveryPlan()` | `registry.mjs`, `projection.mjs`, `startup-failure.mjs` |
| `usr/share/dsh-desktop/lib/plugin-manager/safe-mode.mjs` | Isolated safe-mode profile bootstrap & server | `ensureSafeModeProfile()`, `startSafeModeServer()`, `dispatchAction()` | `paths.mjs`, `recovery-view.mjs` |
...
| `usr/share/dsh-desktop/lib/plugin-manager/startup-failure.mjs` | Log parsing, failure classification & provenance | `parseStartupFailure()`, `reportStartupFailure()`, `checkRecentLogFailures()` | `paths.mjs` |
...
## Entry points
- `usr/local/bin/dsh-desktop`: User desktop GUI launcher (Wayland/X11 detection, Electron launch wrapper).
- `usr/local/bin/dsh-desktop-daemon`: Background harness service supervisor (launches Node with `--expose-internals`).
