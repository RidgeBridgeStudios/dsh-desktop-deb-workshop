export const SUPPORTED_LOCALES = ['en', 'zh']

export const LOCALES = {
  en: {
    nav: 'Plugin market',
    title: 'Plugin market',
    intro: 'Install dsh-market to browse, search, install, and manage community plugins inside DSH Desktop.',
    community: 'dsh-market is maintained by its community. Installing and using community plugins requires network access, and those plugins are not reviewed by DSH Desktop.',
    version: 'Recommended version',
    install: 'Install plugin market',
    installing: 'Installing plugin market…',
    installingHint: 'This can take a few minutes. Keep DSH Desktop open while pnpm downloads and configures the plugin.',
    installed: 'Plugin market installed',
    installedHint: 'Restart Harness once to load the complete market interface.',
    restart: 'Restart Harness',
    restarting: 'Restarting…',
    restartRequired: 'Restart required',
    restartRequiredHint: 'Restart Harness from the DSH Desktop menu to finish applying the change.',
    retry: 'Try again',
    incomplete: 'The previous installation is incomplete. Run the installer again to repair it.',
    failed: 'Plugin market could not be installed.',
    statusFailed: 'Could not read installation status.',
    repository: 'View dsh-market on GitHub',
    futureUpdates: 'After installation, dsh-market will notify you when its own updates are available.',
    managementTab: 'Plugin market',
    managementIntro: 'Manage the optional dsh-market integration installed by DSH Desktop.',
    installedVersion: 'Installed version',
    uninstall: 'Uninstall plugin market',
    uninstalling: 'Uninstalling plugin market…',
    uninstallingHint: 'Removing dsh-market from the profile. Other plugins are not affected.',
    uninstallConfirmTitle: 'Uninstall plugin market?',
    uninstallConfirmDesc: 'Only dsh-market will be removed. Other plugins installed through the market will remain installed.',
    uninstallConfirmNote: 'Restart Harness after removal to finish unloading the market interface.',
    cancel: 'Cancel',
    removed: 'Plugin market uninstalled',
    removedHint: 'dsh-market has been removed. Restart Harness to finish.',
    uninstallFailed: 'Plugin market could not be uninstalled.',
    safeMode: 'Safe Mode',
    safeModeSummary: 'All third-party plugins are temporarily disabled. Sessions, settings, credentials, and workspaces are unchanged. Remove a problematic plugin, then return to normal mode.',
    safeModeSafetyNote: 'Nothing is deleted automatically. Removing a plugin is your explicit choice.',
    safeModeNoPlugins: 'There are no removable third-party plugins in this profile.',
    safeModeSelectPlugins: 'Select plugins to remove',
    safeModeSelectAll: 'Select all',
    safeModeRemoveSelected: 'Remove selected plugins',
    safeModeRemoving: 'Removing…',
    safeModeRestart: 'Still {count} blocking group(s) remain. Continue?',
    safeModeExit: 'Exit Safe Mode and restart',
    safeModeQuit: 'Quit DSH Desktop',
    safeModeUpgradeAll: 'Upgrade {count} plugin(s) with updates',
    safeModeUpgrading: 'Upgrading…',
    recoveryTitle: 'A plugin is preventing startup',
    recoveryNoCandidate: 'No specific plugin could be identified. Enter Safe Mode to disable all third-party plugins and keep using the Agent.',
    recoveryRemove: 'Remove this plugin and continue',
    recoveryRemoveMany: 'Remove these {count} plugins and continue',
    recoveryEnterSafeMode: 'Enter Safe Mode',
    recoveryUpgrade: 'Upgrade plugin and restart',
    recoveryUpgrading: 'Upgrading…',
    recoveryUninstall: 'Uninstall plugin',
    recoveryRetryChecks: 'Retry update checks',
    recoveryAutoProcess: 'Auto-recover ({upgrades} upgrades, {removals} removals)',
    recoveryLog: 'Open Harness log',
    recoveryTechnical: 'View technical details',
    recoverySafetyNote: 'Your workspaces, sessions, model settings, and other plugins will not be removed.',
    externalComponentsNotice: 'This plugin may install system components outside DSH Desktop. DSH Desktop does not manage systemd units, autostart entries, or cron jobs; review the plugin documentation before removing it.',
    noCompatibleUpdate: 'No compatible update is available. Uninstall is the recommended action; you can check again in case a market update has landed.',
    trayDesktop: 'DSH Desktop',
    trayStatusRunning: '● DSH Status: Running',
    trayStatusOffline: '○ DSH Status: Offline',
    trayRunning: 'Running',
    trayOffline: 'Offline',
    trayTooltip: 'DSH Desktop ({status})',
    trayHide: 'Hide to System Tray',
    trayOpen: 'Open DSH Desktop',
    restartDshAction: 'Restart DeepSeek Harness (DSH)',
    restartDesktopApp: 'Restart Desktop App',
    restartBoth: 'Restart Both (Desktop & DSH)',
    quitDesktopApp: 'Quit Desktop App',
    quitEverything: 'Quit Everything (Desktop & DSH)',
    menuFile: 'File',
    menuEdit: 'Edit',
    menuView: 'View',
    menuHelp: 'Help',
    menuReload: 'Reload',
    menuForceReload: 'Force Reload',
    menuDocumentation: 'DeepSeek Harness Documentation',
    menuWorkshop: 'DSH Desktop Workshop',
    menuAbout: 'About DSH Desktop',
    restartingBackend: 'Restarting DeepSeek Harness backend...',
    restartingBoth: 'Restarting DeepSeek Harness & Desktop App...',
    loadingSubtitle: 'Sandboxed Desktop Workspace',
    connectingDaemon: 'Connecting to local daemon...',
    connectionTimeout: "Failed to connect to DeepSeek Harness daemon after 15s. Ensure 'dsh-desktop-daemon' is running.",
    openRecovery: 'Open Recovery Troubleshooter',
    recoveryLead: 'A startup failure was detected. A third-party plugin prevented DeepSeek Harness from launching normally.',
    recoveryResolve: 'Resolve Issue',
    recoveryRetry: 'Retry Startup',
    menuSwitchProfile: 'Switch Profile…',
    profilePickerTitle: 'DeepSeek Harness Profiles',
    profilePickerIntro: 'Select, create, or restore an isolated profile before launching Harness.',
    profilePickerEmpty: 'No profiles found.',
    profileCreate: 'Create Profile',
    profileCreateTitle: 'Create New Profile',
    profileRename: 'Rename Profile',
    profileRenameTitle: 'Rename Profile',
    profileDelete: 'Delete Profile',
    profileDeleteTitle: 'Delete Profile',
    profileDeleteConfirm: 'Delete profile "{name}"?',
    profileDeleteConfirmHint: 'Type the profile name to confirm. A safety snapshot will be taken first.',
    profileDeleteBusy: 'Deleting profile…',
    profileDeleteFailed: 'Failed to delete profile.',
    profileActiveBadge: 'Active',
    profileUse: 'Use Profile',
    profileUseAndRestart: 'Use Profile and Restart',
    profileCancel: 'Cancel',
    profileInternalName: 'Internal Name',
    profileDisplayName: 'Display Name',
    profileColor: 'Badge Color',
    profileSnapshots: 'Snapshots',
    profileSnapshotCreate: 'Create Snapshot',
    profileSnapshotCreated: 'Snapshot created.',
    profileNoSnapshots: 'No snapshots available for this profile.',
    profileLastUsed: 'Last used: {date}',
    profileCreatedAt: 'Created: {date}',
    profileRestore: 'Restore Snapshot',
    profileRestoreTitle: 'Restore Profile Snapshot',
    profileRestoreConfirm: 'Restore snapshot into "{name}"?',
    profileRestoreConfirmHint: 'Type the profile name to confirm.',
    profileRestoreSafetyNote: 'A safety backup of the current state will be taken before restoring.',
    profileRestoreBusy: 'Restoring snapshot…',
    profileRestoreFailed: 'Failed to restore snapshot.',
    profileRestoreOk: 'Snapshot restored successfully.',
    profileNameInvalid: 'Profile name must contain only lowercase letters, digits, dots, underscores, or dashes.',
    profileNameReserved: 'This profile name is reserved.',
    profileNameTaken: 'A profile with this name already exists.',
    trayProfileLabel: 'Profile: {name}'
  },
  zh: {
    nav: '插件市场',
    title: '插件市场',
    intro: '安装 dsh-market，在 DSH Desktop 内浏览、搜索、安装并管理社区插件。',
    community: 'dsh-market 由社区维护。安装和使用社区插件需要联网，这些插件不由 DSH Desktop 审核。',
    version: '推荐版本',
    install: '安装插件市场',
    installing: '正在安装插件市场…',
    installingHint: '下载和配置可能需要几分钟，请保持 DSH Desktop 处于打开状态。',
    installed: '插件市场已安装',
    installedHint: '重启一次 Harness，即可加载完整的插件市场界面。',
    restart: '重启 Harness',
    restarting: '正在重启…',
    restartRequired: '需要重启',
    restartRequiredHint: '请从 DSH Desktop 菜单重启 Harness 以完成更改。',
    retry: '重试',
    incomplete: '上一次安装没有完成，请重新运行安装以修复。',
    failed: '插件市场安装失败。',
    statusFailed: '无法读取安装状态。',
    repository: '在 GitHub 查看 dsh-market',
    futureUpdates: '安装后，dsh-market 会在有新版本时提示并提供升级。',
    managementTab: '插件市场',
    managementIntro: '管理由 DSH Desktop 安装的可选 dsh-market 集成。',
    installedVersion: '当前版本',
    uninstall: '卸载插件市场',
    uninstalling: '正在卸载插件市场…',
    uninstallingHint: '正在从 profile 中移除 dsh-market，其他插件不会受到影响。',
    uninstallConfirmTitle: '卸载插件市场？',
    uninstallConfirmDesc: '只会移除 dsh-market。通过插件市场安装的其他插件将继续保留。',
    uninstallConfirmNote: '移除完成后需要重启 Harness，插件市场界面才会完全退出。',
    cancel: '取消',
    removed: '插件市场已卸载',
    removedHint: 'dsh-market 已移除，请重启 Harness 完成卸载。',
    uninstallFailed: '插件市场卸载失败。',
    safeMode: '安全模式',
    safeModeSummary: '已暂时停用所有第三方插件。会话、设置、凭据和工作区均未改动。可卸载有问题的插件后再返回正常模式。',
    safeModeSafetyNote: '不会自动删除任何内容。卸载插件需要你明确选择。',
    safeModeNoPlugins: '当前 profile 中没有可卸载的第三方插件。',
    safeModeSelectPlugins: '选择要卸载的插件',
    safeModeSelectAll: '全选',
    safeModeRemoveSelected: '卸载所选插件',
    safeModeRemoving: '正在卸载…',
    safeModeRestart: '仍有 {count} 组阻断问题，是否继续？',
    safeModeExit: '退出安全模式并重启',
    safeModeQuit: '退出 DSH Desktop',
    safeModeUpgradeAll: '一键升级 {count} 个有更新的插件',
    safeModeUpgrading: '正在升级…',
    recoveryTitle: '有插件导致启动失败',
    recoveryNoCandidate: '无法定位具体插件。可进入安全模式停用所有第三方插件，继续使用 Agent。',
    recoveryRemove: '卸载此插件并继续检测',
    recoveryRemoveMany: '卸载这 {count} 个插件并继续检测',
    recoveryEnterSafeMode: '进入安全模式',
    recoveryUpgrade: '升级插件并重启',
    recoveryUpgrading: '正在升级…',
    recoveryUninstall: '卸载插件',
    recoveryRetryChecks: '重新检查更新',
    recoveryAutoProcess: '一键自动处理（升级 {upgrades}，卸载 {removals}）',
    recoveryLog: '打开 Harness 日志',
    recoveryTechnical: '查看技术详情',
    recoverySafetyNote: '你的工作区、会话、模型设置和其他插件不会被移除。',
    externalComponentsNotice: '此插件可能在 DSH Desktop 之外安装系统组件。DSH Desktop 不管理系统单元、自启动项或定时任务；卸载前请查阅插件文档。',
    noCompatibleUpdate: '没有可用的兼容更新。建议卸载该插件；也可以重新检查，看看市场上是否已有新版本。',
    trayDesktop: 'DSH Desktop',
    trayStatusRunning: '● DSH 状态：运行中',
    trayStatusOffline: '○ DSH 状态：已离线',
    trayRunning: '运行中',
    trayOffline: '离线',
    trayTooltip: 'DSH Desktop（{status}）',
    trayHide: '隐藏至系统托盘',
    trayOpen: '打开 DSH Desktop',
    restartDshAction: '重启 DeepSeek Harness (DSH)',
    restartDesktopApp: '重启桌面应用',
    restartBoth: '同时重启（桌面应用与 DSH）',
    quitDesktopApp: '退出桌面应用',
    quitEverything: '完全退出（桌面应用与 DSH）',
    menuFile: '文件',
    menuEdit: '编辑',
    menuView: '视图',
    menuHelp: '帮助',
    menuReload: '重新加载',
    menuForceReload: '强制重新加载',
    menuDocumentation: 'DeepSeek Harness 文档',
    menuWorkshop: 'DSH Desktop 工作坊',
    menuAbout: '关于 DSH Desktop',
    restartingBackend: '正在重启 DeepSeek Harness 后端…',
    restartingBoth: '正在重启 DeepSeek Harness 与桌面应用…',
    loadingSubtitle: '沙箱桌面工作区',
    connectingDaemon: '正在连接本地守护进程…',
    connectionTimeout: "连接 DeepSeek Harness 守护进程超时（15秒）。请确保 'dsh-desktop-daemon' 正在运行。",
    openRecovery: '打开故障恢复疑难解答',
    recoveryLead: '检测到启动故障。某个第三方插件阻止了 DeepSeek Harness 正常启动。',
    recoveryResolve: '解决问题',
    recoveryRetry: '重试启动',
    menuSwitchProfile: '切换 Profile…',
    profilePickerTitle: 'DeepSeek Harness Profile 管理',
    profilePickerIntro: '在启动 Harness 之前选择、创建或恢复隔离的 Profile。',
    profilePickerEmpty: '未找到任何 Profile。',
    profileCreate: '创建 Profile',
    profileCreateTitle: '创建新 Profile',
    profileRename: '重命名 Profile',
    profileRenameTitle: '重命名 Profile',
    profileDelete: '删除 Profile',
    profileDeleteTitle: '删除 Profile',
    profileDeleteConfirm: '确定删除 Profile "{name}" 吗？',
    profileDeleteConfirmHint: '请输入 Profile 名称以确认。删除前将自动生成安全快照。',
    profileDeleteBusy: '正在删除 Profile…',
    profileDeleteFailed: '删除 Profile 失败。',
    profileActiveBadge: '当前使用',
    profileUse: '使用此 Profile',
    profileUseAndRestart: '使用此 Profile 并重启',
    profileCancel: '取消',
    profileInternalName: '内部名称',
    profileDisplayName: '显示名称',
    profileColor: '标记颜色',
    profileSnapshots: '历史快照',
    profileSnapshotCreate: '创建快照',
    profileSnapshotCreated: '快照已创建。',
    profileNoSnapshots: '当前 Profile 暂无可用快照。',
    profileLastUsed: '上次使用：{date}',
    profileCreatedAt: '创建时间：{date}',
    profileRestore: '恢复快照',
    profileRestoreTitle: '恢复 Profile 快照',
    profileRestoreConfirm: '确定将快照恢复至 "{name}" 吗？',
    profileRestoreConfirmHint: '请输入 Profile 名称以确认。',
    profileRestoreSafetyNote: '恢复前将自动对当前状态进行安全备份。',
    profileRestoreBusy: '正在恢复快照…',
    profileRestoreFailed: '恢复快照失败。',
    profileRestoreOk: '快照已成功恢复。',
    profileNameInvalid: 'Profile 名称仅支持小写字母、数字、点、下划线及连字符。',
    profileNameReserved: '该 Profile 名称为保留名称。',
    profileNameTaken: '同名的 Profile 已存在。',
    trayProfileLabel: 'Profile：{name}'
  }
}

export function resolveLocale(preference) {
  if (preference === 'zh' || preference === 'en') return preference
  if (typeof preference === 'string' && preference.toLowerCase().startsWith('zh')) return 'zh'
  return 'en'
}

export function translate(locale, key) {
  const language = resolveLocale(locale)
  return LOCALES[language][key] ?? LOCALES.en[key] ?? key
}

export function formatMessage(locale, key, values = {}) {
  return translate(locale, key).replace(/\{(\w+)\}/gu, (_, name) => String(values[name] ?? ''))
}
