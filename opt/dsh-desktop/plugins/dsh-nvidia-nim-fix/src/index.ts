 ...
 */
export function isNimEndpoint(url: string | URL): boolean {
  try {
    const raw = typeof url === 'string' ? url : url instanceof URL ? url.href : (url as any)?.url;
    if (!raw) return false;
 ...
 */
export function transformNimRequestBody(body: NimRequestBody): { body: NimRequestBody; modified: boolean } {
  let modified = false;
...
export function apply(ctx?: any): void {
  if ((globalThis as any)[SENTINEL] || (globalThis.fetch as any)?.__isNimFix) {
    return;
  }
  ...
  (patchedFetch as any).__isNimFix = true;
  (globalThis as any)[SENTINEL] = true;
